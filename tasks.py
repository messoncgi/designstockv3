...
# Note: Esta função NÃO é 'async def'
def perform_designi_download_task(
    ...
    try:
        with sync_playwright() as p: # Usando sync_playwright
            try:
                ...
                if not os.path.exists(temp_file_path) or os.path.getsize(temp_file_path) == 0:
                     # ==================================
                     # === CORREÇÃO: Remover 'await' ===
                     # ==================================
                     failure_reason = download.failure() # Chamada síncrona, sem await
